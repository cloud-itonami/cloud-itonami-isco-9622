(ns handyman.actor
  "HandymanPracticeActor — the ISCO-08 9622 community handyman actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one handyman operation request
  ...
  The unconditional invariant: the HandymanPracticeAdvisor can never
  directly commit a record the HandymanPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [handyman.advisor :as advisor]
            [handyman.governor :as governor]
            [handyman.store :as store]))

(defn build-graph
  "Build a compiled HandymanPracticeActor graph. `store` implements
  `handyman.store/Store`. `advisor` implements
  `handyman.advisor/Advisor` (defaults to `mock-advisor`).
